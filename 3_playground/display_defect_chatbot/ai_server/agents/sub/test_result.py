...
async def test_result_node(state: SubAgentInput) -> dict:
    """테스트결과 에이전트: 조회 + LLM 분석 → suspect_rows + analysis"""
    rows = await query_test_results(state["product_id"])
...
{json.dumps(rows, ensure_ascii=False, default=str, indent=2)}"""
    ...
    return {"test_result": {"suspect_rows": result.suspect_rows, "analysis": result.analysis}}
